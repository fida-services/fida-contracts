...
import Fida.Contract.TestToolbox.TypedValidators (InsurancePolicy, PiggyBank, fidaCardStatusNFT, piggyBank, fidaCardNFT, fidaCardNegateNFT,fidaCardStatusNegateNFT, fidaCardFromInt, iinfoBox, ppInfoBox, pbBox, insurancePolicy)
 ...
 where
  mkTx iinfo =
    mconcat
      ...
      , payToRef tv (InlineDatum iinfo) value
      ]
...
buyFidaCard iid fcid users@Users{..} = do
  ...
  withBox @PiggyBank (pbBox iid fcid) tv $ \box -> do
...
payPremium iid users@Users{..} = do
  let tv = insurancePolicy iid
  sp <- spend policyHolder $ adaValue 400_000_000
 ...
 where
  ...
  spendPPaymentInfo =
    mconcat
      ...
      <>
      [ payToRef (PiggyBankAddress address) (InlineDatum $ PBankPremium ppInfoPremiumAmountPerPiggyBank) (adaValue ppInfoPremiumAmountPerPiggyBank)
      | address <- ppInfoPiggyBanks
      ])
